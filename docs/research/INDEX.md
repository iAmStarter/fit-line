...
| 2026-07-04 | impl-phase-1 | LINE getContent `GET https://api-data.line.me/v2/bot/message/{messageId}/content` returns Blob; reply API `POST https://api.line.me/v2/bot/message/reply` with Flex bubble (min: type=bubble, body=box). Postback data max 300 chars; quick-reply cameraRoll in message envelope. CacheService TTL max 600s (10 min). OcrMetrics 25-key defined in OVERVIEW §5 (activeCaloriesKcal, totalCaloriesKcal key fields for phase 1 calorie rule). Test harness mocks ready (CacheService, UrlFetchApp); phase 1 overrides needed. **Zero blockers; all contracts confirmed.** | `/Users/istd/fit-webhook/docs/research/impl-phase-1.md` |
| 2026-07-04 | impl-phase-2 | Postback event shape: `event.source.userId` (Line identifier) + `event.postback.data` (`action=confirm&id=<shortId>` format, parsed via regex). Stash retrieval: `retrieveOcr(id)` returns OcrMetrics or null. Phase 1 **does NOT stash messageId** — **BLOCKING:** implementer must decide (A: Phase 1 refactor to stash alongside OcrMetrics, or B: Phase 2 generates via `Utilities.getUuid()`). Recommendation: Option A per PLAN risk #4. Sheet schema: submissions (14 cols: messageId, userId, name, activityType, activityDateISO, submittedAtISO, activeCaloriesKcal, totalCaloriesKcal, distanceKm, source, confidence, status="recorded", rejectReason="", imageHash=""); employees (3 cols: userId, name, registeredAtISO); append by column order; nulls → empty string ''. CacheService `.remove()` needed for idempotent stash deletion after successful write. Timestamps via `new Date().toISOString()`. Success card: green #1e9e57, no emoji, chip+label. Error cards: stash-miss → "หมดเวลา ส่งรูปใหม่" (quick-reply cameraRoll); write-fail → "บันทึกไม่สำเร็จ ลองใหม่" (no stash delete). SpreadsheetApp mocks already in test/setup.ts. **One blocking decision + zero other blockers.** | `/Users/istd/fit-webhook/docs/research/impl-phase-2.md` |
| 2026-07-04 | impl-phase-3 | SHA-256 Blob: `Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, blob)` → signed byte[] → convert hex via `(b & 0xff).toString(16).padStart(2,'0')`. imageHash lookup: scan submissions by column index; O(n) cost < OCR. Rate-limit: no atomic increment; get/parse/+1/put(key,val,60s); boundary 5-pass/6-block. LockService: lock in postback path (not image), `waitLock(10000)`, check messageId before write, releaseLock in finally. imageHash threading: Option A (compute at image-time, stash in StashedContext, write at postback) — breaks phase boundary but keeps cost gate clean. Three reject cards (duplicate-image, cooldown, lock-timeout) all red #d64545, no emoji. Order in image path: rate-limit → hash dedup → OCR (all cheaper gates first). Mocks ready (Utilities.computeDigest, LockService, CacheService stateful, SpreadsheetApp). **Zero blockers; 5 interface questions flagged (see §10).** | `/Users/istd/fit-webhook/docs/research/impl-phase-3.md` |
